...
Note that the IRFs LP estimator, unlike IRFs calculated from AR(-DL) models, do not exhibit the mean reversion property (which is a direct consequence of being robust AR model misspecification), which mean that for large horizons, $H$, estimates will be highly inaccurate due to elevated finite sample variance of the estimator (i.e. LP is less efficient than correctly specified AR-DL). 
Additionally, the fact that LP is instrumented additionally increases the variance of the estimator.\footnote{The premium of LP on bias over efficiency, not on minimizing meansquared error loss.} 
For this reason I restrict the projection horizon to 20, $H=20$ (5 years of quarterly observations), but note that the significance of the projections for larger horizons should be viewed skeptically. 
...
In order to find some orthogonal variation in projection of 
...
The impact of hawkishness of FOMC on the effect of a change in projected inflation becomes prominent after 3rd quarter and is negative, that is the additional 2 hawk members of FOMC reduce the response to inflation by 0.3 percentage points starting from the 3rd quarter. Note that the effect of hawks on response to inflation becomes not distinguishable from zero (at 10\% significance level) starting from the 7th quarter.
...
Also, from \vref{tab:LP_short} one can note that the 

\begin{table}[!hbtp]\centering \footnotesize
  \begin{threeparttable}
    \caption{Estimates of Short LP Taylor Rule(selected quarters)}
    \label{tab:LP_short}
...
\begin{figure}[!htbp]\centering
  ...
  \end{subfigure}
      {\begin{flushleft}\scriptsize\textit{Notes}: This figure reports the responses of the $(r-r^*)_t$ to an increase in the Tealbook deflator inflation forecast of 1 p.p. and unemployment gap forecast of $-1$ p.p.. The subfigure \ref{fig:LP_long:average_inflation} reports the response of $(r-r^*)_t$ to projected deflator inflation for the $\mathit{HAWK}$ index equal to the sample average; \ref{fig:LP_long:differential_inflation} is the addition to the response in case there are 2 (out of 12 in total) additional consistent hawks in the FOMC. Subfigures \ref{fig:LP_long:average_gap} and \ref{fig:LP_long:differential_gap} report the same for the increase in projected GDP gap for 1p.p. The shaded areas correspond to 68\%, 90\% and 95\% confidence bands calculated with HAC estimator with \citet{Andrews1991} weighting.\end{flushleft}}
\end{minipage}
\end{figure}
...
\begin{figure}\centering 
  \begin{minipage}{0.5\textwidth}\centering
    \caption{In-sample Predictive Ability $(R^2)$ of the Models}
    \label{fig:r_square}
...
The  \vref{fig:pathes_predictions} shows the point-predictions made using the model \vref{eq:LP1} based on the FOMC composition and tealbook projections of inflation and output gap. I restrict the prediction horizon to 12 quarters, which represent the 

The predicted paths are hard to analyze, so one of the ways to look at them, which is inspired by the size-persistence tradeoff in heterogeneous agent new keynesian model (HANK) by \citet{KMV2018}, is to estimate the size and persistence of monetary policy. Contrary from \citet{KMV2018}, I define the size is defined as a mean of predicted rate path.\footnote{\citet{KMV2018} define size as a sum of deviation of rate of interest from natural rate for the whole duration of the policy, i.e. $\sum_{t=1}^n r_t-r^*$, the size in my definition is a \citet{KMV2018} defined scaled by $1/13$.}
...
\begin{figure}[!hpbt]\centering
  \begin{minipage}{1\textwidth}\centering
    ...
    \end{subfigure}
    {\begin{flushleft}\scriptsize \textit{Notes:} This figure shows the size and persistence calculated as in \vref{sec:size_persistence} for 2 specifications. The prediction period is restricted to start at 1988 Q3 for both specifications.\end{flushleft}} 
    \end{minipage}

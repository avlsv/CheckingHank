...
dates <-
  ...
  select(year_quarter)
pos_dates <-
  dates$year_quarter[1:length(dates$year_quarter)] |> as.character()

residuals_tbl <- tibble(date = pos_dates)
...
for (i in 0:20) {
  ...
  residuals <-
    tibble(residuals = reg$residuals,
           date =  pos_dates[as.numeric(names(reg$residuals))])
  
  
  
  
  residuals_tbl <-
    left_join(residuals_tbl, residuals, by = join_by(date))
  
}

residuals_tbl <- residuals_tbl |> na.omit()
colnames(residuals_tbl) <-
  c("date",  as.character(0:20))
residuals_ts <-
  residuals_tbl |> mutate(date = yearquarter(date)) |> tsibble() |> filter_index("1987" ~
                                                                                   .)
...
residuals_size_persistence_tbl <-
  residuals_long |>
  filter(period <= 15) |>
  group_by(date) |>
  summarize(size = sum(value),
            persistence =
              acf(value, lag.max = 1, plot = F)$acf[2])

residuals_size_persistence_ts <-
  residuals_size_persistence_tbl |>
  mutate(date = yearquarter(date)) |>
  tsibble()

residuals_size_persistence_ts |> autoplot(size)
residuals_size_persistence_ts |> autoplot(persistence)
...
residuals_long_ts
  ...
  ggplot(
    ...
    geom_density_ridges_gradient(scale = 1.5, rel_min_height = 0.03) +
    theme_ridges() +
    theme(legend.position = "none")
  
  

  ggplot(residuals_size_persistence_ts,
         aes(
           x = size,
           y = persistence,
           color = yq(date)
    ...
    geom_point(size = 1.3) +
    labs(x = "Size", y = "Persistence", color = "Year") +
    theme_light()
